...
2. 三档价格窗口和价格锚点；
3. 三个关键时刻：现在预订、9 月 21 日完成订单、发货前开始 coaching；
4. 精简产品证明；
5. FAQ 和人工客服。
...
| Cohort test | 400–600 | 比较 Leads 类型与素材角度 | 支付完成率和订单完成率达到预设阈值 |
...
- 不写“发货后开始 coaching”；
- 支付前必须说明：任何时候都可退款；
- 必须提醒使用能接收通知的邮箱，预订和正式订单使用相同邮箱；
- 价格未解锁时不展示虚构的 MSRP 或 `$300–$400` 参考区间；价格解锁后只展示已确认的 Reward 窗口和真实的 `$319` launch-day 锚点；
- PSG 只使用经过证据和法务确认的措辞。
