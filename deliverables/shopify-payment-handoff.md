...
> ✅ **退款政策已裁决（2026-08-04）：采用页面通用口径。**
> `$9` 在发射前后均可申请原路全额退款，任何理由，与页面承诺 "Refundable any time, for any reason — before or after launch" 完全一致。本单 C/D 条已按此更新；确认邮件与客服口径必须使用同一表述，不得出现「上线后不退款」类措辞。
...
- [ ] 邮件没有写成“众筹结束后自动退 $9”
- [ ] 邮件明确：$9 在发射前后均可申请原路全额退款，任何理由（需人工联系客服发起）

### D. 退款与异常

- [ ] 退款申请（上线前后均可）：客服核验 Shopify 订单后原路退款，订金资格随退款失效
- [ ] Kickstarter 上线后未 pledge：$9 仍可申请原路全额退款（与页面承诺一致）
- [ ] 众筹失败或取消：全额原路退款
